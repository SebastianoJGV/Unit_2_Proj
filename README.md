...
Figure 7. The code showcasing the bitmaps used to turn specific lights on.

To solve the requirement of displaying numbers in a different number system in criterion A, we are displaying kanji. To do so, we are using an 8 x 8 grid of LED lights. To determine which lights must be turned on per kanji, we developed bitmaps for each number. As you can see, each bitmap starts with setting it up with “[8] [8]” (8 by 8). Then, whenever an LED light should be turned on, “1” is used. Lights that must be turned off are shown with “0”.
